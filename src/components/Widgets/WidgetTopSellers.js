...
class WidgetTopSellers extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			tabIndex: 0,
			data: [ 40000, 50000, 38000, 37000, 35000, 37000],
			labels: ["Computer", "Phone", "Clothing", "Household", "Accessories","Grooming"],
			data2: [ 42000, 40000, 35000, 25000, 30000, 33000],
			labels2: ["Acme", "Globex", "Soylent", "Initech", "Hooli","Vehement"],
			// legendLabel: 
			visitorStats: [
	...
	/** main function */
	render() {
		const { tabIndex, data, labels, data2, labels2, visitorStats } = this.state;
		const { productData,theme } = this.props;
		return (
...
export default withStyles(null, { withTheme: true })(WidgetTopSellers);